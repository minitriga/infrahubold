...
A node that has a profile assigned, will get the values of its attributes inherited from the assigned profile. The attribute values of a node that were inherited from a profile can be overridden.
...
When you create a device in a source of truth, such as Infrahub, you might want to have a template for the different device types that you deploy in your network. A device template should contain the details of the manufacturer, type, platform, module slots, interfaces etc.

Creating a device profile capturing these common set of device attributes, would allow you to create such a templated that can be used to create multiple instances of such a device.
...
- Required attributes of a Node must have a default value defined in the schema, if you want to define these atrributes using profiles
